...
(( ! ${+ZSH_RELOAD_COMPLETIONS_ZCOMPDUMP} )) &&
    typeset ZSH_RELOAD_COMPLETIONS_ZCOMPDUMP="${ZDOTDIR:-$HOME}/.zcompdump"

(( ! ${+ZSH_RELOAD_COMPLETIONS_STATE_DIR} )) &&
    typeset ZSH_RELOAD_COMPLETIONS_STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/zsh-reload-completions"
...
typeset _ZSH_RELOAD_COMPLETIONS__SUBSCRIPTION="$ZSH_RELOAD_COMPLETIONS_STATE_DIR/subscribers/$$"

mkdir -p "${ZSH_RELOAD_COMPLETIONS_ZCOMPDUMP:h}"
mkdir -p "${ZSH_RELOAD_COMPLETIONS_STATE_DIR}"
mkdir -p "${_ZSH_RELOAD_COMPLETIONS__SUBSCRIPTION:h}"
: >| "$_ZSH_RELOAD_COMPLETIONS__SUBSCRIPTION"

_zsh_reload_completions__unregister() {
    # Clean up PID file
    if [[ -f "$_ZSH_RELOAD_COMPLETIONS__SUBSCRIPTION" ]]; then
        rm -f "$_ZSH_RELOAD_COMPLETIONS__SUBSCRIPTION"
    fi
}
